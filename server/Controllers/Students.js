...
export const fetchAllDetails = async (req, res) => {
  try {
    ...
    let query = `
      ...
      FROM students
      LEFT JOIN internships ON students.Roll_No = internships.Roll_No
      LEFT JOIN projects ON students.Roll_No = projects.Roll_No
      WHERE 
        students.Department = ? AND
        (students.Year_of_Joining = ? OR students.Year_of_Joining IS NULL) AND
        (students.Year_of_Passing = ? OR students.Year_of_Passing IS NULL) AND
        (internships.Internship_Type = ? OR internships.Internship_Type IS NULL) AND
        (projects.Project_Type = ? OR projects.Project_Type IS NULL)
    `;

    let queryParams = [Department, Year_of_Joining, Year_of_Passing, Internship_Type, Project_Type];
    ...
    // Fetching student, internship, and project details using LEFT JOINs
